...
/* eslint-disable no-undef, no-useless-escape, no-control-regex, brace-style, no-unused-vars */
...
const AMOUNT_REGEX =
  /(?:THB|บาท|Amount)\s*[:]?\s*([0-9]{1,3}(?:,[0-9]{3})*(?:\.[0-9]{2})|[0-9]+\.[0-9]{2})/i;
...
class OCRTemplatesManager {
  ...
  async processDocument (base64Data, type = 'bank_slip', options = {}) {
    try {
      ...
      const amountMatch = normalized.match(AMOUNT_REGEX) || normalized.match(ALT_AMOUNT_REGEX);
      if (amountMatch) {
...
async function handleSlipVerification (request, env) {
  try {
    ...
    await pendingKV.put(`slip_${depositId}`, JSON.stringify(pendingData), { expirationTtl: 86400 });
...
async function handleBankEmailNotification (request, env) {
  try {
    ...
    if (!message || !message.data) {
      logger.info('Invalid bank email webhook payload');
    ...
    if (!emailDetails.success) {
      logger.info('Email parsing failed:', emailDetails.error);
    ...
    if (depositId && userId) {
      try {
        ...
        if (bankingResponse.ok) {
          ...
          logger.info(`Successfully verified and confirmed deposit ${depositId}`);
        } else {
          const error = await bankingResponse.json();
          throw new Error(`Banking worker failed to confirm deposit ${depositId}: ${error.error}`);
        }
      } catch (confirmError) {
        logger.error('Deposit confirmation error:', confirmError);
      ...
      logger.info('No matching slip found for email amount:', emailDetails.data.amount);
    }

    return successResponse({ message: 'Bank email webhook processed successfully' });
  } catch (error) {
    logger.error('Bank Email Webhook Error:', error);
    await logSecurityError(error, request, env);
...
async function findMatchingSlip (emailData, env) {
  try {
    ...
    logger.error('Error finding matching slip:', error);
    return {};
  }
}
...
async function updateDepositStatus (depositId, status, note, env) {
  try {
    ...
    if (!response.ok) {
      ...
      logger.error('Failed to update deposit status:', error);
    }
  } catch (error) {
...
async function logSecurityError (error, request, env) {
  try {
    ...
    logger.error('Security error logging failed:', logError);
  }
}
...
async function blockIPTemporarily (ip, duration, reason, env) {
  ...
  try {
    ...
    logger.error('Failed to temporarily block IP:', error);

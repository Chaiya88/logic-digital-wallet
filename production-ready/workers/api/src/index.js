...
export default {
  async fetch (request, env, ctx) {
    ...
    try {
      ...
      let securityCheck;
      try {
        securityCheck = await detectSuspiciousActivity(request, env);
      } catch (error) {
      ...
      logger.error('API Worker Error:', error);
      return new Response(
        JSON.stringify({
...
async function initiateFiatDeposit (request, userId, env, corsHeaders) {
  try {
    ...
    logger.error('Fiat deposit initiation error:', error);
    return new Response(
      JSON.stringify({
        success: false,
...
async function submitDepositSlip (request, userId, env, corsHeaders) {
  try {
    ...
    logger.error('Deposit slip submission error:', error);
    return new Response(
      JSON.stringify({
        success: false,
...
async function initiateCryptoWithdrawal (request, userId, env, corsHeaders) {
  try {
    ...
    logger.error('Crypto withdrawal initiation error:', error);
    return new Response(
      JSON.stringify({
        success: false,
...
async function handleBankEmailWebhook (request, env) {
  try {
    ...
    env.SECURITY.fetch(
      ...
      logger.error('Failed to forward bank email webhook:', error);
    });
...
async function getWalletAddress (userId, env, corsHeaders) {
  try {
    ...
    if (result.success && result.wallet.wallet_address) {
      ...
      await env.DOGLC_WALLET_CACHE.put(`address_${userId}`, result.wallet.wallet_address, {
        expirationTtl: 3600
      });
...
async function processNewBlock (blockchainData, env) {
  try {
    ...
    logger.error('New block processing error:', error);
    return { success: false, error: error.message };
  }
}

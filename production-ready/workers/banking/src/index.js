...
export default {
  async fetch (request, env, ctx) {
    ...
    try {
      ...
      logger.error('Banking Worker Error:', error);
  ...
  async scheduled (env, ctx) {
    ctx.waitUntil(
      handleDailyReset(env).catch(error => {
        logger.error('Scheduled daily reset failed:', error);
      })
    );
  }
};
...
async function confirmTHBDeposit (request, env) {
  try {
    ...
    const tx = await env.MAIN_WALLET_DB.prepare(
      'SELECT * FROM transactions WHERE id = ? AND to_user = ? AND (status = \'pending_payment\' OR status = \'pending_verification\')'
    )
    ...
    if (configStr) {
      try {
        ...
        commissionPct = cfg?.commission?.deposit || 0;
      } catch {}
    }
    ...
    try {
      await env.MAIN_WALLET_DB.prepare('UPDATE wallets SET balance = balance + ? WHERE user_id = ?')
        .bind(doglcAmount, userId)
        .run();
      if (feeDoglc > 0.0001) {
        // Record fee transaction (internal)
        const feeId = `fee_${generateUUID()}`;
        await env.MAIN_WALLET_DB.prepare(
          'INSERT INTO transactions (id, from_user, amount, status, type, note, currency, created_at) VALUES (?, ?, ?, \'completed\', \'fee\', ?, \'DOGLC\', ? )'
        )
          .bind(feeId, userId, feeDoglc, 'Deposit commission fee', new Date().toISOString())
          .run();
      }

      await env.MAIN_WALLET_DB.prepare(
        'UPDATE transactions SET status = \'completed\', confirmed_at = ?, note = ? WHERE id = ?'
      )
        .bind(
          new Date().toISOString(),
          `Completed THB Deposit. Received ${doglcAmount.toFixed(4)} DOGLC.`,
          depositId
        )
        .run();
...
async function initiateUSDTWithdrawal (request, env, ctx) {
  try {
    ...
    if (configStr) {
      try {
        const cfg = JSON.parse(configStr);
        withdrawCommissionPct = cfg?.commission?.withdraw || 0;
      } catch {}
    }
    ...
    try {
      await env.MAIN_WALLET_DB.prepare(
        ...
        .bind(doglcAmount, doglcAmount, userId)
        .run();

      await env.MAIN_WALLET_DB.prepare(
        'INSERT INTO transactions (id, from_user, amount, status, type, note, currency, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)'
      )
        .bind(
          withdrawalId,
          userId,
          ...
          'pending_withdrawal',
          'crypto_withdrawal',
          `Withdraw ${usdtAmount.toFixed(4)} USDT (fee ${feeUsdt.toFixed(4)}) to ${usdtAddress}`,
          'USDT',
          new Date().toISOString()
        )
        .run();
      if (feeUsdt > 0.0000001) {
        const feeId = `fee_${generateUUID()}`;
        await env.MAIN_WALLET_DB.prepare(
          'INSERT INTO transactions (id, from_user, amount, status, type, note, currency, created_at) VALUES (?, ?, ?, \'completed\', \'fee\', ?, \'USDT\', ?)'
        )
          .bind(feeId, userId, feeUsdt, 'Withdrawal commission fee', new Date().toISOString())
          .run();
      }
...
async function selectReceivingAccount (amount, env) {
  try {
    ...
    const bankAccounts = JSON.parse(bankAccountsStr);
    const candidates = bankAccounts.filter(
      acc => acc.status === 'active' && acc.currentDailyTotal + amount <= acc.dailyLimit
    );
    if (candidates.length === 0) {
      return {
        success: false,
        error: 'All receiving accounts are temporarily unavailable. Please try again later.'
      };
    }
    ...
    const scored = candidates.map(acc => {
      ...
      const score =
        capacityRatio * 0.45 + (1 - utilization) * 0.15 + priorityNorm * 0.25 + reliability * 0.15;
      return { acc, score };
    });
    ...
    const updatedAccounts = bankAccounts.map(a =>
      a.accountId === selected.accountId
        ? { ...a, currentDailyTotal: a.currentDailyTotal + amount }
        : a
    );
    await env.DOGLC_CONFIG.put('BANK_ACCOUNTS', JSON.stringify(updatedAccounts));
    return { success: true, account: selected };
  } catch (error) {
    logger.error('Account Selection Error:', error);
    return {
      success: false,
      error: 'An internal error occurred while selecting a payment account.'
    };
  }
}
...
async function handleDailyReset (env) {
  try {
    ...
    logger.info(`Daily Reset Successful: Reset daily totals for ${resetAccounts.length} accounts.`);
    return new Response(`Reset daily totals for ${resetAccounts.length} accounts.`, {
      status: 200
    });
  } catch (error) {
    logger.error('Daily Reset Cron Job Error:', error);
...
async function processCryptoPayout (withdrawalId, usdtAddress, usdtAmount, env) {
  try {
    logger.info(
      `Processing payout of ${usdtAmount} USDT to ${usdtAddress} for withdrawal ${withdrawalId}`
    );
    // Simulate network/chain delay
    await new Promise(resolve => setTimeout(resolve, 30000));

    const isSuccess = Math.random() > 0.1; // simulated success rate
    const tx = await env.MAIN_WALLET_DB.prepare('SELECT * FROM transactions WHERE id = ?')
      .bind(withdrawalId)
      .first();
    if (!tx) {
      logger.warn(`Transaction ${withdrawalId} not found during payout processing.`);
      return;
    }
    ...
    try {
      if (isSuccess) {
        ...
        `
          ...
          .bind(
            `Sent ${usdtAmount.toFixed(4)} USDT to ${usdtAddress}`,
            new Date().toISOString(),
            withdrawalId
          )
          .run();
        ...
        `
          ...
          .bind(tx.amount, new Date().toISOString(), tx.from_user)
          .run();
        ...
        `
          ...
          .bind(
            'Crypto payout failed. Funds have been returned.',
            new Date().toISOString(),
            withdrawalId
          )
          .run();
        ...
        `
          ...
          .bind(tx.amount, tx.amount, new Date().toISOString(), tx.from_user)
          .run();

        logger.info(`Withdrawal ${withdrawalId} failed. User has been refunded.`);
      }
      await env.MAIN_WALLET_DB.prepare('COMMIT').run();
    } catch (dbError) {
      await env.MAIN_WALLET_DB.prepare('ROLLBACK').run();
    ...
    logger.error('processCryptoPayout error:', error);
...
async function getExchangeRate (from, to, env) {
  ...
  try {
    ...
    logger.warn('Could not load exchange rates from config:', error);
  }
...
async function updateTransactionStatus (request, env) {
  try {
    ...
    const queryNote = note ? ', note = ?' : '';
    const params = note ? [status, note, transactionId] : [status, transactionId];

    await env.MAIN_WALLET_DB.prepare(`UPDATE transactions SET status = ? ${queryNote} WHERE id = ?`)
      .bind(...params)
      .run();
...
async function createWallet (request, env) {
  try {
    ...
    const dbResult = await env.MAIN_WALLET_DB.prepare(
      `
    ...
    if (dbResult.success) {
      // Cache wallet data
      await env.DOGLC_WALLET_CACHE.put(`wallet_${userId}`, JSON.stringify(walletData), {
        expirationTtl: 300
      });
...
async function validateTransaction (request, env) {
  try {
    ...
    // Overall validation
    validationResults.valid = Object.values(validationResults.checks).every(
      check => check === true
    );
...
async function validateAddress (request, env) {
  try {
    ...
    if (address.startsWith('doglc1') && address.length === WALLET_ADDRESS_LENGTH) {
      ...
      const walletData = await env.MAIN_WALLET_DB.prepare(
        `
...
async function resolveRecipient (recipient, env) {
  try {
    ...
    const userData = await env.MAIN_WALLET_DB.prepare(
      `
...
async function getDailyTransactionUsage (userId, env) {
  try {
    ...
    const result = await env.MAIN_WALLET_DB.prepare(
      `
...
async function flagSuspiciousTransaction (fromUserId, recipient, amount, reason, env) {
  try {
    ...
    logger.error('Error flagging suspicious transaction:', error);
  }
}
...
async function sendSecurityAlert (type, data, env) {
  try {
    ...
    // Store alert
    await env.SECURITY_TOKENS.put(`alert_${Date.now()}`, JSON.stringify(alertData), {
...
async function sendTransactionNotification (transactionData, status, env) {
  try {
    ...
    logger.error('Transaction notification error:', error);
  }
}
...
async function applyRateLimit (env, key, limit, windowSecs) {
  try {
    ...
    await env.RATE_LIMITS.put(key, JSON.stringify(bucket), {
      expiration: Math.floor(bucket.expiresAt / 1000)
    });
...
async function updateRates (request, env) {
  try {
    ...
    if (withdrawalFeePercent !== undefined) {
      cfg.RATES.withdrawalFeePercent = Number(withdrawalFeePercent);
    }
...
async function adminAddBankAccount (request, env) {
  try {
    ...
    if (!bankName || !accountName || !accountNumber) {
      return errorResponse('Missing bank account fields', 400);
    }
    const cfg = await loadConfig(env);
...
async function adminRemoveBankAccount (request, env) {
  try {
    ...
    return successResponse({
      ...
      remaining: cfg.BANK_ACCOUNTS.length
    });
  } catch (e) {
...
async function getAdminDashboard (env) {
  try {
    ...
    utilization.forEach(u => {
      if (u.utilization >= 90) {
        alerts.push({
          type: 'bank_account_capacity',
          accountId: u.accountId,
          message: `Account ${u.accountId} at ${u.utilization}% of daily limit`
        });
      }
      if (u.status !== 'active') {
        alerts.push({
          ...
          accountId: u.accountId,
          message: `Account ${u.accountId} status = ${u.status}`
        });
      }
    });
    // Pending transactions
    const pendingDeposits = await env.MAIN_WALLET_DB.prepare(
      'SELECT COUNT(*) as c FROM transactions WHERE type=\'fiat_deposit\' AND status IN (\'pending_payment\',\'pending_verification\')'
    ).first();
    const pendingWithdrawals = await env.MAIN_WALLET_DB.prepare(
      'SELECT COUNT(*) as c FROM transactions WHERE type=\'crypto_withdrawal\' AND status=\'pending_withdrawal\''
    ).first();
    const dashboard = {
...
async function loadConfig (env) {
  const cfgStr = await env.DOGLC_CONFIG.get('SYSTEM_CONFIG');
  if (!cfgStr) {
  ...
  if (!cfg.RATES) {
    cfg.RATES = { depositFeePercent: 0, withdrawalFeePercent: 0.5, commissionPercent: 1.0 };
  }
...
async function logBankingOperation (operation, userId, data, env) {
  try {
    ...
    logger.error('Banking operation logging error:', error);
  }
}
...
async function logBankingError (error, context, env) {
  try {
    ...
    logger.error('Error logging banking error:', logError);
  }
}
...
async function processTransactionAsync (transactionData, recipientUserId, env) {
  try {
    ...
    `
      ...
      .bind(new Date().toISOString(), transactionData.id)
      .run();
    ...
    try {
      ...
      `
        ...
        .bind(transactionData.amount, transactionData.from_user)
        .run();
      ...
      `
        ...
        .bind(transactionData.amount, new Date().toISOString(), recipientUserId)
        .run();
    ...
    logger.error('Async transaction processing error:', error);
...
async function performFraudDetection (fromUserId, recipient, amount, env) {
  try {
    ...
    if (recentTx.length > 20) {
      // More than 20 transactions in an hour
  ...
  try {
    ...
    try {
      ...
      `
        ...
        .bind(amount, amount, new Date().toISOString(), userId)
        .run();
...
async function processUnstaking (request, env) {
  try {
    ...
    const stakingData = await env.MAIN_WALLET_DB.prepare(
      `
      SELECT * FROM staking WHERE id = ? AND user_id = ? AND status = 'active'
    `
    )
      .bind(stakingId, userId)
      .first();
    ...
    const daysStaked = Math.floor(
      (Date.now() - new Date(stakingData.start_date).getTime()) / (24 * 60 * 60 * 1000)
    );
    ...
    try {
      ...
      `
        ...
        .bind(currentRewards, new Date().toISOString(), stakingId)
        .run();
      ...
      `
        ...
        .bind(totalReturn, stakingData.amount, new Date().toISOString(), userId)
        .run();
...
async function calculateStakingRewards (request, env) {
  try {
    ...
    const stakingPositions = await env.MAIN_WALLET_DB.prepare(
      `
    ...
    for (const position of stakingPositions.results) {
      const daysStaked = Math.floor(
        (Date.now() - new Date(position.start_date).getTime()) / (24 * 60 * 60 * 1000)
      );
  ...
  try {
    ...
    try {
      ...
      `
        ...
        .bind(claimableRewards, new Date().toISOString(), userId)
        .run();
  ...
  try {
    ...
    if (referrerId) {
      ...
      `
        ...
        .bind(referrerCommission, new Date().toISOString(), referrerId)
        .run();
...
async function getUserAnalytics (request, env) {
  try {
    ...
    const transactions = await env.MAIN_WALLET_DB.prepare(
      `
    ...
    `
      ...
      .bind(userId, userId, startDate.toISOString(), endDate.toISOString())
      .all();
...
async function getSystemAnalytics (env) {
  try {
    ...
    await env.DOGLC_BANK_ANALYTICS.put(cacheKey, JSON.stringify(analytics), { expirationTtl: 600 });
  ...
  try {
    ...
    try {
      ...
      `
        ...
        .bind(totalDeduction, totalDeduction, new Date().toISOString(), userId)
        .run();
  ...
  try {
    ...
    if (success) {
      ...
      `
        ...
        .bind(new Date().toISOString(), withdrawalData.id)
        .run();
    ...
    logger.error('Async withdrawal processing error:', error);
  }
}
...
async function confirmTransaction (request, env) {
  try {
    ...
    const transaction = await env.MAIN_WALLET_DB.prepare(
      `
  ...
  try {
    ...
    try {
      ...
      `
        ...
        .bind(`Cancelled: ${reason || 'User requested'}`, transactionId)
        .run();
      ...
      if (transaction.from_user) {
        ...
        `
          ...
          .bind(refundAmount, refundAmount, new Date().toISOString(), transaction.from_user)
          .run();

  ...
  try {
    ...
    if (env.MAIN_WALLET_DB) {
      try {
        ...
        if (userCount && userCount.count !== undefined) {
          stats.activeUsers = userCount.count;
        }
        if (txCount && txCount.count !== undefined) {
  ...
  try {
    ...
    if (request.method === 'GET') {
      ...
      if (env.MAIN_WALLET_DB && userId !== 'anonymous') {
        try {
          const userPrefs = await env.MAIN_WALLET_DB.prepare(
...
async function getRecentTransactions(env, userId = null, limit = 10) {
  ...
  try {
    let query = `
      SELECT id, from_user, to_user, amount, currency, type, status, note, created_at 
      FROM transactions 
...
async function getSystemStats(env) {
  ...
  try {
    const [userCount, txCount, activeUsers, volume] = await Promise.all([
      env.MAIN_WALLET_DB.prepare('SELECT COUNT(*) as count FROM users').first(),
      env.MAIN_WALLET_DB.prepare('SELECT COUNT(*) as count FROM transactions').first(),
      env.MAIN_WALLET_DB.prepare(
        'SELECT COUNT(DISTINCT from_user) as count FROM transactions WHERE created_at > datetime("now", "-1 day")'
      ).first(),
      env.MAIN_WALLET_DB.prepare(
        'SELECT COALESCE(SUM(amount), 0) as volume FROM transactions WHERE created_at > datetime("now", "-1 day")'
      ).first()
    ]);

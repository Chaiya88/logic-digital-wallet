...
// Note: OCRTemplatesManager is embedded directly in this file to avoid import issues
...
// --- OCR Templates Manager (embedded) ---
const BANK_KEYWORDS = [
  'SCB',
  'KBank',
  'Krungthai',
...
const AMOUNT_REGEX =
  /(?:THB|บาท|Amount)\s*[:]?\s*([0-9]{1,3}(?:,[0-9]{3})*(?:\.[0-9]{2})|[0-9]+\.[0-9]{2})|([0-9]{1,3}(?:,[0-9]{3})*(?:\.[0-9]{2})|[0-9]+\.[0-9]{2})\s*(?:THB|บาท)/i;
const ALT_AMOUNT_REGEX = /([0-9]{1,3}(?:,[0-9]{3})*(?:\.[0-9]{2}))/;
const DATE_REGEX = /(\d{2}[\/-]\d{2}[\/-]\d{4})|(\d{4}[\/-]\d{2}[\/-]\d{2})/;
const ACCOUNT_REGEX = /(\d{3}-?\d{1}-?\d{5}-?\d{1}|\d{10,12})/;
...
class OCRTemplatesManager {
  ...
  /**
   * Processes a base64-encoded document and extracts banking slip information.
   * @param {string} base64Data - The base64-encoded slip image data.
   * @param {string} [type='bank_slip'] - The type of document to process.
  ...
  async processDocument(base64Data, type = 'bank_slip', options = {}) {
    try {
      ...
      const valid = score >= 60 && errors.filter(e => e !== 'account_not_found').length === 0;

      return {
        success: true,
        result: {
          validation: { valid, errors: valid ? [] : errors },
  ...
  async _decodeToText(cleaned) {
    try {
      ...
      const nonPrintableRatio =
        (text.match(/[^\x09\x0A\x0D\x20-\x7Eก-๙]/g) || []).length / text.length;
      if (nonPrintableRatio > 0.4) return '';
      return text;
    } catch {
      return '';
    }
  ...
  _heuristicDecodeFallback(cleaned) {
    const b64Portion = cleaned.slice(0, 500);
...
async function verifyWebhookSignature(request, env, signature, timestamp) {
  try {
    ...
    const key = await crypto.subtle.importKey(
      'raw',
      new TextEncoder().encode(env.GMAIL_WEBHOOK_SECRET),
      { name: 'HMAC', hash: 'SHA-256' },
      false,
      ['sign']
    );
    const sigBuf = await crypto.subtle.sign('HMAC', key, enc);
    const digest = Array.from(new Uint8Array(sigBuf))
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (!authenticated) {
      return new Response(JSON.stringify({ error: 'Unauthorized', message: 'Auth failed' }), {
        status: 401,
  ...
  try {
    ...
    if (!ocrResult.success || !ocrResult.result.validation.valid) {
      ...
      return errorResponse(
        `${t('slip_failed', useLang)}: ${ocrResult.error || ocrResult.result.validation.errors.join(', ')}`,
        400,
        useLang
      );
    }
    ...
    let pendingKV = env.SLIP_VERIFICATION || env.PENDING_VERIFICATIONS;
    if (!pendingKV) {
      console.warn(
        'Warning: Neither SLIP_VERIFICATION nor PENDING_VERIFICATIONS is defined in env. Slip verification data will not be stored.'
      );
    } else {
      await pendingKV.put(`slip_${depositId}`, JSON.stringify(pendingData), {
        expirationTtl: 86400,
      });
    }
    ...
    if (depositId && userId) {
      try {
        ...
        const bankingResponse = await env.BANKING.fetch(
          new Request('https://banking-worker/fiat/deposit/confirm', {
            method: 'POST',
            headers: {
...
function parseBankEmail(emailContent) {
  try {
    ...
    async function findMatchingSlip(emailData, env) {
      try {
        const pendingKV = env.SLIP_VERIFICATION || env.PENDING_VERIFICATIONS;
        // Try to filter keys by amount if possible (requires storing amount in key or using metadata)
        // If not possible, filter by recent date to reduce number of keys loaded
        const now = new Date();
        const oneDayAgo = new Date(now.getTime() - 24 * 60 * 60 * 1000).toISOString();

        // List only slips created in the last 24 hours (if supported by KV)
        const list = await pendingKV.list({ prefix: 'slip_' });
        const matchingCandidates = [];

        for (const key of list.keys) {
          // Optionally, skip keys not created recently if key metadata is available
          // If not, load and check createdAt inside slip data
          ...
          if (pendingDataStr) {
            ...
            // Only check slips created in the last 24 hours
            if (slipDate >= new Date(oneDayAgo)) {
...
async function updateDepositStatus(depositId, status, note, env) {
  try {
    ...
    const response = await env.BANKING.fetch(
      new Request('https://banking-worker/transactions/update-status', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-Internal-API': env.INTERNAL_API_KEY,
...
async function createUserSession(request, env) {
  try {
    ...
    // Store session
    await env.USER_SESSIONS.put(`session_${sessionToken}`, JSON.stringify(sessionData), {
...
async function createSecurityIncident(request, env) {
  try {
    ...
    `
      ...
      .bind(incidentId, type, severity, description, 'open', incidentData.created_at)
      .run();
...
async function getSecuritySummary(env) {
  try {
    ...
    // Cache for 5 minutes
    await env.SECURITY_TOKENS.put(cacheKey, JSON.stringify(summary), { expirationTtl: 300 });
...
async function getThreatAnalysis(env) {
  try {
    ...
    await env.SECURITY_TOKENS.put(cacheKey, JSON.stringify(analysis), { expirationTtl: 900 });
...
async function getAuditTrail(request, env) {
  try {
    ...
    // Cache for 2 minutes
    await env.ENHANCED_AUDIT_LOGS.put(cacheKey, JSON.stringify(result), { expirationTtl: 120 });
...
async function assessTransactionRisk(transactionData, env) {
  ...
  if (transactionData.type === 'fiat_deposit') {
    ...
    if (recentFiatDeposits > 3) {
      // More than 3 deposits in 24h
      riskScore += 20;
...
async function analyzeTransactionVelocity(userId, transactionData, env) {
  try {
    ...
    return {
      suspicious:
        hourlyTransactions > suspiciousThresholds.hourly_count ||
...
async function analyzeTransactionPatterns(userId, transactionData, env) {
  try {
    ...
    for (let i = 1; i < recentTransactions.length; i++) {
      const diff =
        new Date(recentTransactions[i - 1].created_at) - new Date(recentTransactions[i].created_at);
      timeDiffs.push(diff);
    }

    const avgTimeDiff = timeDiffs.reduce((sum, diff) => sum + diff, 0) / timeDiffs.length;
    if (avgTimeDiff < 60000 && recentTransactions.length > 5) {
...
async function logSecurityError(error, request, env) {
  try {
    ...
    await env.ENHANCED_AUDIT_LOGS.put(`security_error_${Date.now()}`, JSON.stringify(errorData), {
      expirationTtl: 86400 * 7,
    });
  } catch (logError) {
...
/**
 * Returns a standardized successful JSON response for API endpoints.
 * @param {Object} payload - The response data to include in the body.
 * @param {string} [lang=DEFAULT_LANG] - Language code for internationalization.
 * @param {number} [status=200] - HTTP status code for the response.
 * @returns {Response} - A Response object with success, lang, and payload fields.
 * Example payload: { message: string, data?: Object, ... }
 */
function successResponse(payload, lang = DEFAULT_LANG, status = 200) {
   ...
   * @param {string} [lang=DEFAULT_LANG] - The language code for the response.
   * @param {Object} [extra={}] - Additional properties to include in the response body.
   * @returns {Response} - A Response object with error details in JSON format.
   */

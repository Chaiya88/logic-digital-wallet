  ...
  const { depositId, userId } = await findMatchingSlip(emailDetails.data, env);
    ...
    if (depositId && userId) {
      try {
        ...
        const bankingResponse = await env.BANKING.fetch(new Request('https://banking-worker/fiat/deposit/confirm', {
          method: 'POST',
          headers: { 
...
async function updateDepositStatus(depositId, status, note, env) {
  try {
    ...
    const response = await env.BANKING.fetch(new Request('https://banking-worker/transactions/update-status', {
      method: 'POST',
      headers: { 
        'Content-Type': 'application/json', 
        'X-Internal-API': env.INTERNAL_API_KEY 

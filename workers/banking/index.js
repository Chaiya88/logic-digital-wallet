...
async function confirmTHBDeposit(request, env) {
  try {
    ...
    const tx = await env.MAIN_WALLET_DB.prepare(`SELECT * FROM transactions WHERE id = ? AND to_user = ? AND (status = 'pending_payment' OR status = 'pending_verification')`).bind(depositId, userId).first();
    if (!tx) {
    ...
    try {
      await env.MAIN_WALLET_DB.prepare(`UPDATE wallets SET balance = balance + ? WHERE user_id = ?`).bind(doglcAmount, userId).run();
      if (feeDoglc > 0.0001) {
        // Record fee transaction (internal)
        const feeId = `fee_${generateUUID()}`;
        await env.MAIN_WALLET_DB.prepare(`INSERT INTO transactions (id, from_user, amount, status, type, note, currency, created_at) VALUES (?, ?, ?, 'completed', 'fee', ?, 'DOGLC', ? )`)
          .bind(feeId, userId, feeDoglc, 'Deposit commission fee', new Date().toISOString()).run();
      }
      
      await env.MAIN_WALLET_DB.prepare(`UPDATE transactions SET status = 'completed', confirmed_at = ?, note = ? WHERE id = ?`)
...
async function initiateUSDTWithdrawal(request, env, ctx) {
  ...
  const usdtAmount = grossUsdt - feeUsdt;
    ...
    await env.MAIN_WALLET_DB.prepare('BEGIN TRANSACTION').run();
    try {
      await env.MAIN_WALLET_DB.prepare(`UPDATE wallets SET balance = balance - ?, frozen_balance = frozen_balance + ? WHERE user_id = ?`)
        .bind(doglcAmount, doglcAmount, userId).run();

      await env.MAIN_WALLET_DB.prepare(`INSERT INTO transactions (id, from_user, amount, status, type, note, currency, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
      ...
      if (feeUsdt > 0.0000001) {
        const feeId = `fee_${generateUUID()}`;
        await env.MAIN_WALLET_DB.prepare(`INSERT INTO transactions (id, from_user, amount, status, type, note, currency, created_at) VALUES (?, ?, ?, 'completed', 'fee', ?, 'USDT', ?)`)
...
async function processCryptoPayout(withdrawalId, usdtAddress, usdtAmount, env) {
  try {
    ...
    const tx = await env.MAIN_WALLET_DB.prepare(`SELECT * FROM transactions WHERE id = ?`).bind(withdrawalId).first();
    if (!tx) {
...
async function getAdminDashboard(env) {
  try {
    ...
    const pendingDeposits = await env.MAIN_WALLET_DB.prepare(`SELECT COUNT(*) as c FROM transactions WHERE type='fiat_deposit' AND status IN ('pending_payment','pending_verification')`).first();
    const pendingWithdrawals = await env.MAIN_WALLET_DB.prepare(`SELECT COUNT(*) as c FROM transactions WHERE type='crypto_withdrawal' AND status='pending_withdrawal'`).first();
    const dashboard = {
